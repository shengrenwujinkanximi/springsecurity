...
//@Configuration
public class QuartzSchedulerTaskConfig {
    @Autowired
    private SchedulerTasksRepository schedulerTasksRepository;

    @Bean(name = "jobDetail")
    public MethodInvokingJobDetailFactoryBean jobDetailFactoryBean(SchedulerQuartzJobClass quartzJob1) {
        ...
        jobDetail.setName("tasksName");// 设置任务的名字
        jobDetail.setGroup("groupName");// 设置任务的分组，这些属性都可以存储在数据库中，在多任务的时候使用

        /*
         * 为需要执行的实体类对应的对象
         */
        jobDetail.setTargetObject(quartzJob1);
    ...
    public SchedulerFactoryBean schedulerFactory(CronTriggerFactoryBean jobTrigger) {
        ...
        // 注册触发器
        bean.setTriggers(jobTrigger.getObject());
        return bean;
    }
}
